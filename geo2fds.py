...
class geo2fds:
    ...
    def __init__(self, filename,time,level,title=''):
        ...
        for i in range(8):
            print(i+1,self.dataset.read(i+1)[1,1])

        for i in range(8):
            print(i+1,self.dataset.read(i+1)[1,2])
        self.topo()
    ...
    def topo(self):
        ...
        xMesh = multimesh.multiMesh([  yLR,yUL,xUL, xLR,zMin,zMax],[50,50,50],[3,3,])
        mesh, mult =xMesh.meshGen()

        self.allObst = [mesh,mult]

        obst = "&OBST XB= {},{},{},{},{},{} ,ID={}, SURF_ID='{}'/ "
        ...
        for row in range(x):
            for col in range(y):
                ...
                self.allObst.append(obst.format(xUL+(row*horz), xUL+((row+1)*horz),yUL+(col*vert),yUL+((col+1)*vert),zMin,DEM[row,col],id,sID))

    def fire(self,hrrpua=500,tStart =1, tEnd=60,xFire=500, yFire = 500):
        assert tStart<tEnd
